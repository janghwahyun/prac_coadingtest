...
function solution(numbers, hand) {
    ...
    numbers.map(number => {
        if (left.includes(number)) { //왼손으로 눌러야할 때
            answer += 'L';
            LPos = [left.indexOf(number), 0];
            
        } else if (right.includes(number)) { //오른손으로 눌러야할 때
            answer += 'R';
            ...
            if (LDist > RDist) { //오른손이 거 가까이 있으면
                answer += 'R'; //R을 추가하고
                RPos = index; //오른손을 타겟 번호위치로 이동
            } else if (RDist > LDist) { //왼손이 더 가까이 있으면
                answer += 'L';
                LPos = index;
            } else { //거리가 같을때
                if (hand === 'right') { //hand가 right이면 
                    answer += 'R';
                    RPos = index;
                } else { //hand가 left이면
                    answer += 'L';
